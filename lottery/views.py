...
class LotteryResultByUniqueIDView(APIView):
    """
    API to get complete lottery result data by unique result_id.
    Accepts both GET (query parameter) and POST (request body) methods.
    """
    permission_classes = [AllowAny]
    
    def get(self, request):
        """Handle GET request with result_id as query parameter"""
        ...
        return self._get_lottery_result(result_id)
    
    def post(self, request):
        """Handle POST request with result_id in request body"""
        result_id = request.data.get('result_id')
        
        if not result_id:
            return Response(
                {"error": "result_id is required in request body"},
                status=status.HTTP_400_BAD_REQUEST
            )
        
        return self._get_lottery_result(result_id)
    
    def _get_lottery_result(self, result_id):
        """Private method to fetch and return complete lottery result data"""
        try:
            # Fetch the lottery draw with all related data
            draw = LotteryDraw.objects.select_related('lottery_type').prefetch_related(
                'first_prize', 'second_prize', 'third_prize', 'fourth_prize',
                'fifth_prizes', 'consolation_prizes', 'sixth_prizes', 
                'seventh_prizes', 'eighth_prizes', 'ninth_prizes', 'tenth_prizes'
            ...
            if not draw.result_declared:
                return Response(
                    {"error": "Result not yet declared for this lottery"},
                    status=status.HTTP_404_NOT_FOUND
                )
            ...
            return Response(
                {"error": "Lottery result not found with the provided result_id"},
                status=status.HTTP_404_NOT_FOUND
            )
        except Exception as e:
            return Response(
...
def get_lottery_result_by_id(request):
    """
    Function-based view to get complete lottery result data by unique result_id.
    ...
    try:
        ...
        result_data = {
            "result_id": draw.result_id,
            "lottery_info": {
                "name": draw.lottery_type.name,
                "code": draw.lottery_type.code,
                "price": draw.lottery_type.price,
                "first_prize_amount": draw.lottery_type.first_prize_amount
            },
            "draw_info": {
                "draw_number": draw.draw_number,
                "draw_date": draw.draw_date.isoformat(),
                ...
                "is_today": draw.is_today,
                "is_yesterday": draw.is_yesterday
            },
            "prizes": {}
        }
        ...
        # Add First Prize
        if hasattr(draw, 'first_prize') and draw.first_prize:
            result_data["prizes"]["first"] = {
                "amount": float(draw.first_prize.amount),
                "ticket_number": draw.first_prize.ticket_number,
                "place": draw.first_prize.place
            }
        
        # Add Second Prize
        if hasattr(draw, 'second_prize') and draw.second_prize:
            result_data["prizes"]["second"] = {
                "amount": float(draw.second_prize.amount),
                "ticket_number": draw.second_prize.ticket_number,
                "place": draw.second_prize.place
            }
        
        # Add Third Prize
        if hasattr(draw, 'third_prize') and draw.third_prize:
            result_data["prizes"]["third"] = {
                "amount": float(draw.third_prize.amount),
                "ticket_number": draw.third_prize.ticket_number,
                "place": draw.third_prize.place
            }
        
        # Add Fourth Prize
        if hasattr(draw, 'fourth_prize') and draw.fourth_prize:
            result_data["prizes"]["fourth"] = {
                "amount": float(draw.fourth_prize.amount),
                "ticket_number": draw.fourth_prize.ticket_number,
                "place": draw.fourth_prize.place
            }
        
        # Add Fifth Prizes (Multiple)
        if draw.fifth_prizes.exists():
            result_data["prizes"]["fifth"] = [
                {
                    "amount": float(prize.amount),
                    "ticket_number": prize.ticket_number,
                    "place": prize.place
                }
                for prize in draw.fifth_prizes.all()
            ]
        
        # Add Consolation Prizes
        if draw.consolation_prizes.exists():
            result_data["prizes"]["consolation"] = [
                {
                    "amount": float(prize.amount),
                    "ticket_number": prize.ticket_number
                }
        ...
        # Add Sixth Prizes
        if draw.sixth_prizes.exists():
            result_data["prizes"]["sixth"] = [
                {
                    "amount": float(prize.amount),
                    "number": prize.number
                }
                for prize in draw.sixth_prizes.all()
            ]
        
        # Add Seventh Prizes
        if draw.seventh_prizes.exists():
            result_data["prizes"]["seventh"] = [
                {
                    "amount": float(prize.amount),
                    "number": prize.number
                }
                for prize in draw.seventh_prizes.all()
            ]
        
        # Add Eighth Prizes
        if draw.eighth_prizes.exists():
            result_data["prizes"]["eighth"] = [
                {
                    "amount": float(prize.amount),
                    "number": prize.number
                }
                for prize in draw.eighth_prizes.all()
            ]
        
        # Add Ninth Prizes
        if draw.ninth_prizes.exists():
            result_data["prizes"]["ninth"] = [
                {
                    "amount": float(prize.amount),
                    "number": prize.number
                }
                for prize in draw.ninth_prizes.all()
            ]
        
        # Add Tenth Prizes
        if draw.tenth_prizes.exists():
            result_data["prizes"]["tenth"] = [
                {
                    "amount": float(prize.amount),
                    "number": prize.number
                }
                for prize in draw.tenth_prizes.all()
            ]
...
class LotteryResultSearchView(APIView):
    """
    API to search lottery results by various criteria and return complete data.
    This complements the unique ID search by allowing searches by lottery code, draw number, etc.
    """
    permission_classes = [AllowAny]
    
    def post(self, request):
        """Handle POST request with search criteria"""
        lottery_code = request.data.get('lottery_code')
        draw_number = request.data.get('draw_number')
        draw_date = request.data.get('draw_date')
        
        if not any([lottery_code, draw_number, draw_date]):
            return Response(
                {"error": "At least one search criteria is required (lottery_code, draw_number, or draw_date)"},
                status=status.HTTP_400_BAD_REQUEST
            )
        
        try:
            # Build filter conditions
            filter_conditions = {'result_declared': True}
            
            if lottery_code:
                filter_conditions['lottery_type__code'] = lottery_code
            
            if draw_number:
                filter_conditions['draw_number'] = draw_number
            
            if draw_date:
                filter_conditions['draw_date'] = draw_date
            
            # Fetch matching draws
            draws = LotteryDraw.objects.filter(**filter_conditions).select_related(
                'lottery_type'
            ).prefetch_related(
                'first_prize', 'second_prize', 'third_prize', 'fourth_prize',
            ...
            if not draws.exists():
                return Response(
                    {"error": "No lottery results found matching the search criteria"},
                    status=status.HTTP_404_NOT_FOUND
                )
